...
impl CargoToml {
	...
	where
		...
		F: FnOnce() -> Result<()>,
	{
		self.with_amended_manifest(|toml| {
			let lib = toml.get_mut("lib")
				.ok_or(anyhow::anyhow!("lib section not found"))?;
			let crate_types = lib.get_mut("crate-type")
				.ok_or(anyhow::anyhow!("crate-type section not found"))?;
			let crate_types = crate_types.as_array_mut()
	...
	where
		F: FnOnce() -> Result<()>
	{
		self.with_amended_crate_types(|crate_types| {
