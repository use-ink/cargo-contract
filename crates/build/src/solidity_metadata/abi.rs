...
fn primitive_ty(ty_def: &TypeDefPrimitive, msg: &str) -> Result<&'static str> {
    let sol_ty = match ty_def {
        ...
        // NOTE: (@davidsemakula) Rust strings are UTF-8, while solidity string literals
        // only support ASCII characters, so we have to use unicode literals.
        // However, the Solidity ABI spec uses string for both, and claims that `string`
        // is a "dynamic sized unicode string assumed to be UTF-8 encoded"
